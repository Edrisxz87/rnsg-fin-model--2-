...
# ------------------------------------------------------------------
# 1. Interest Calculation Functions (Compound for Homeowners, Simple for Investors)
# ------------------------------------------------------------------
...
def monthly_investor_payout(total_investor_interest, term_years):
    """Calculate monthly investor payout (simple interest)."""
...
# ------------------------------------------------------------------
# 2. Project Count per Month Function
# (Months 1–6 fixed; then growth: 20% for months 7–12, 10% for months 13–24, 5% for months 25–36)
# ------------------------------------------------------------------

def get_projects_count(month):
    """
    Returns the number of projects launched in a given month.
    Fixed for months 1-6:
      - Month 1 & 2: 0
      - Month 3: 1
      - Month 4: 2
      - Month 5: 4
...
# ------------------------------------------------------------------
# 4. Scenario Table Generation (Base and Default)
# ------------------------------------------------------------------
...
def group_cashflow_by_year(df):
    ...
    each containing a horizontal 3x4 DataFrame for that year.
    """
    groups = {}
    for year in range(1, 4):
        ...
        # Create lumpset for this subset: only months where a lump repayment occurred
        # For simplicity, we'll use the actual Month numbers from the subset.
        lumpset = set(subset["Month"])
        groups[year] = chunk_12_months_horizontally(subset, lumpset)
    return groups

# ------------------------------------------------------------------
# 7. Print Two Tables Side by Side (Console Output)
# ------------------------------------------------------------------
...
def main():
    ...
    # Build lumpsets for coloring lump repayment months
    ...
    # Split cashflow data into yearly subsets
